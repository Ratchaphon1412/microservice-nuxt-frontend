<template>
  <div
    class="mx-auto max-w-2xl items-center space-x-2 px-4 sm:px-6 lg:max-w-7xl lg:px-8 bg-white"
  >
    <!-- content card -->
    <nav>
      ...
      <div class="p-6">
        <ul
          class="hidden text-sm font-medium text-center divide-x rounded-lg shadow sm:flex divide-gray-700 text-gray-400"
        >
          <li class="w-full">
            <button
              @click="chooseGender('Women')"
              class="relative inline-flex items-center justify-center w-full h-full overflow-hidden font-mono font-medium tracking-tighter text-white bg-white rounded-l-lg border-white border-2 group group-hover:w-56 group-hover:h-56 text-black hover:text-white"
            >
              <span
                class="absolute w-0 h-0 transition-all duration-500 ease-out bg-gray-800 rounded-full group-hover:w-full group-hover:h-96"
              ></span>
              <span class="relative text-black group-hover:text-white text-xl"
                >Women</span
              >
            </button>
          </li>
          <li class="w-full">
            <button
              @click="chooseGender('Men')"
              class="relative inline-flex items-center justify-center w-full h-full overflow-hidden font-mono font-medium tracking-tighter text-white bg-white border-white border-2 group group-hover:w-56 group-hover:h-56 text-black hover:text-white"
            >
              <span
                class="absolute w-0 h-0 transition-all duration-500 ease-out bg-gray-800 rounded-full group-hover:w-full group-hover:h-96"
              ></span>
          ...
          <li class="w-full">
            <button
              @click="chooseGender('Kids')"
              class="relative inline-flex items-center justify-center w-full h-12 overflow-hidden font-mono font-medium tracking-tighter text-white bg-white rounded-r-lg border-white border-2 group group-hover:w-56 group-hover:h-56 text-black hover:text-white"
            >
              <span
                class="absolute w-0 h-0 transition-all duration-500 ease-out bg-gray-800 rounded-full group-hover:w-full group-hover:h-96"
              ></span>
      ...
      <!-- Filter -->
      <div
        class="font-bold p-4 bg-white m-5 rounded-lg flex flex-rows text-black border-4 border-gray-900"
      >
        <span class="text-xl text-black"> Filter : </span>
        <div v-for="filter in filterData" :key="filter">
          <div
            v-if="filter != '' && filter != 0"
            class="mx-3 text-center rounded-lg bg-gray-200 px-2 py-1 flex flex-rows"
          >
            ...
            <button @click="clearItemFilter($event, filter)">
              <svg
                class="w-2 h-2 m-2 hover:ring-2"
                fill="#000000"
                height="200px"
                width="200px"
                version="1.1"
    ...
    <div class="flex">
      <aside
        class="flex flex-col w-72 px-5 py-8 overflow-y-auto bg-white border-r rounded-md rtl:border-r-0 rtl:border-l dark:bg-gray-900 dark:border-gray-700"
      >
        <h1 class="text-2xl font-bold text-start text">
        ...
        <div class="px-3 py-4 overflow-y-auto">
          ...
          <div class="space-y-2 font-medium">
            <button
              ...
              aria-controls="dropdown-tops"
              data-collapse-toggle="dropdown-tops"
            >
              <span class="flex-1 ml-3 text-left text-xl whitespace-nowrap"
                >All Product</span
              >
            </button>
          </div>
          ...
          <div class="space-y-2 font-medium">
            <button
              @click="selectCategory('Outerwear')"
              type="button"
              class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700"
              aria-controls="dropdown-Outerwear"
              data-collapse-toggle="dropdown-Outerwear"
            >
              <svg
                version="1.1"
                class="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white"
                    ...
                    <g>
                      <g>
                        <g>
                          <path
                            fill="#000000"
                            d="M74.8,182c-1.6-0.4-2.6-1.9-2.3-3.5l9.3-43.4c0.3-1.6,1.9-2.6,3.5-2.3c1.6,0.3,2.6,1.9,2.3,3.5l-9.3,43.4C78,181.1,76.7,182.4,74.8,182z"
                          />
                        </g>
                        <g>
                          <g>
          ...
          <div class="space-y-2 font-medium">
            <button
              @click="selectCategory('Bottoms')"
              type="button"
              class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700"
              aria-controls="dropdown-BOTTOMS"
              data-collapse-toggle="dropdown-BOTTOMS"
            >
              <svg
                class="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white"
                aria-hidden="true"
                ...
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                viewBox="0 0 466.389 466.389"
                xml:space="preserve"
              >
                <path
                  d="M306.865,3.215c-0.893-2.331-3.362-3.651-5.8-3.083c-44.399,10.296-91.339,10.296-135.741,0
        ...
        <div class="border-t border-gray-200">
          ...
          <RadioGroup v-model="filterData.selectedColor" class="mt-4">
            <RadioGroupLabel class="sr-only">Choose a color</RadioGroupLabel>
            <div
              class="lg:grid lg:max-w-7xl lg:grid-cols-4 lg:grid-rows-[auto,auto,1fr] p-2 gap-2"
            >
              <RadioGroupOption
                @click="filterColor(color.name)"
                as="template"
              ...
              >
                <div
                  :class="[
                    ...
                    'relative -m-0.5 cursor-pointer flex items-center justify-center rounded-full p-0.5 focus:outline-none',
                  ]"
                >
                  ...
                  <span
                    aria-hidden="true"
                    class="h-8 w-8 rounded-full border border-black border-opacity-10 hover:ring-2"
            ...
            <button class="grid grid-cols-3 gap-4 md:grid-cols-3">
              ...
              >
                <div
                  :class="[
                    size,
                    active && checked ? 'ring-1 ring-[#112D4E]' : '',
                    !active && checked ? '' : '',
                    ' hover:ring-2 border-black border-opacity-10 group relative flex items-center justify-center rounded-md border text-lg font-medium uppercase hover:bg-gray-50 focus:outline-none sm:flex-1',
                  ]"
                >
                  <RadioGroupLabel as="span" class="p-2">{{
                    size
                  }}</RadioGroupLabel>
                </div>
              </RadioGroupOption>
                        ...
                        <div class="flex gap-1">
                            <ul>
                                <li>
                                    ...
                                    <label class="ml-2 text-gray-600">less than ฿500</label>
                                </li>
                                <li>
                                    <input type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                    <label class="ml-2  text-gray-600">฿500 - ฿999</label>
                                </li>
                                <li>
                                    <input type="checkbox" class="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                    <label class="ml-2 text-gray-600">more than ฿1000</label>
                                </li>
                            </ul>
                        </div>
                    </div>
        ...
        <div class="border-t border-gray-200 mt-4">
          ...
          <div class="p-4 flex flex-col gap-4">
            <div
              @click="filterCost(cost.cost)"
              class="flex gap-1"
              v-for="cost in listCost"
              :key="cost"
            >
