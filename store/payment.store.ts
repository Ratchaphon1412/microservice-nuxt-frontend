export const apiPayment = defineStore(
  "apiPayment",
  () => {
    let credits = ref([]);
    ...
    async function addCredit(
      nameIn: string,
      numberIn: string,
      expiration_monthIn: string,
      expiration_yearIn: string,
      cityIn: string,
      postalIn: string,
      securityIn: string,
      ...
      const { data } = await paymentFetch(
        URL + "/api/v1/payment/customer/card/",
        {
          method: "POST",
      ...
      if (data.value) {
        ...
        const { addCard } = await paymentFetch(
          URL + "/api/v1/payment/customer/card/",
          {
            method: "PUT",
            body: JSON.stringify({
    ...
    async function getListCard(customer_token: string) {
      const { data } = await paymentFetch(
        URL + "/api/v1/payment/customer/card/?token=" + customer_token,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    async function invoiceList(user_token: string) {
      const { data, error } = await paymentFetch(
        URL + "/api/v1/payment/invoice/?token=" + user_token,
        {
          method: "PUT",
          body: JSON.stringify({
            user_id: user_token,
